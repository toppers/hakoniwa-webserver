...
def main(asset_name, config_path, delta_time_usec, is_conductor, max_delay_time_usec):
    # HTTPサーバーを別スレッドで実行
    ...
    if is_conductor:
        # conductor を起動
        print("INFO: start conductor")
        hakopy.conductor_start(delta_time_usec, max_delay_time_usec)
        time.sleep(0.5)

    # PDUサービスを別スレッドで実行
    pdu_service_thread = threading.Thread(
        target=lambda: run_hako_pdu_service(HakoPduCommWebSocketImpl.get_instance(), asset_name, config_path, delta_time_usec, is_conductor)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_delay_time_usec', type=int, required=False, help='Max delay time in microseconds')
    args = parser.parse_args()

    is_conductor = False
    if args.max_delay_time_usec is not None:
        is_conductor = True


    main(args.asset_name, args.config_path, args.delta_time_usec, is_conductor, args.max_delay_time_usec)
